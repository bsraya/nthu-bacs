...
```{r, echo = FALSE}
max_reptime <- max(verizon$Time)
min_reptime <- min(verizon$Time)
glue("max_time -> {max_reptime} \n min_time -> {min_reptime}.")
```
...
In this part, I am going to split the data set into two parts, ILEC and CLEC.
```{r}
...
\textcolor{red}{
In this case, we have t-value which is far away from 0. This means that what Verizon's repair time is not like what Verizon has advertised.
}
