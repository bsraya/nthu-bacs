...
ii. Run a linear model of x and y points to confirm the R2 value reported by the simulation: `regr <- lm(y ~ x, data=pts) summary(regr)`
...
1. Get values of $\hat{y}$( regression line’s estimates of y, given x): `y_hat <- regr$fitted.values`
...
2. Add segments: `segments(pts$x, pts$y, pts$x, y_hat, col="red", lty="dotted")`
...
iv. Which relationships might not be linear? (do not worry about linearity for rest of this HW)
